...
const LastPaymentsList = ({ loading, error }: LastPaymentsListProps) => {
      ...
      {mockPayments
        ...
        .map((payment) => (
          <Row.Root
            ...
            onClick={() => handlePaymentCard(payment.id)}
          >
            <Row.Driver
              driverPhotoUrl={payment.driverPhotoUrl}
              driverName={payment.driverName}
              showImage={true}
              textColor="#1B556D"
              textFontWeight="700"
            />
            <Row.Contract contract={payment.contractNumber} />
            <Row.Route originState={payment.originState} destinyState={payment.destinyState} />
            <Row.Value value={payment.value} />
            <Row.PaymentDate date={payment.date} />
            <Row.PaymentType paymentType={payment.type} />
          </Row.Root>
        ))}
      </div>
    </div>
