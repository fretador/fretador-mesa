...
export const useFreightController = () => {
  ...
  const loadFreights = async (
    ...
    console.log("Loading Freights with Filters:", filters); // Log dos filtros antes da chamada ao backend
    ...
    try {
      ...
      dispatch(
        fetchFreightsSuccess({ freights: transformedFreights, pageInfo })
      );
    } catch (error: unknown) {
      const errorMessage =
