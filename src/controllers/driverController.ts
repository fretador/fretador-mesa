	...
	const loadDrivers = useCallback(
		async (page: number, limit: number, filter: DriverFilters) => {
			...
			try {
				...
				const transformedDrivers: Driver[] = DriverService.transformDrivers(response.data);
				const pageInfo: PageInfo = DriverService.transformPageInfo(response.pageInfo);
				dispatch(fetchDriversSuccess({ drivers: transformedDrivers, pageInfo, status }));
			} catch (error: unknown) {
				const errorMessage =
